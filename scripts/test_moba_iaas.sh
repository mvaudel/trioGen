...
do
    ...
    java -Xmx32G -cp bin/triogen-0.3.0-beta/triogen-0.3.0-beta.jar no.uib.triogen.cmd.association.LinearModel -g $vcfFolder/$chr.vcf.gz -gf 1 -f $trioFile -p $phenoFile -pn $phenoName -cv $covariates -o $outputFolder/chr_$chr.gz -test

done

echo "Processing chromosome X"

java -Xmx32G -cp bin/triogen-0.3.0-beta/triogen-0.3.0-beta.jar no.uib.triogen.cmd.association.LinearModel -g $vcfFolder/X.vcf.gz -gf 1 -f $trioFile -p $phenoFile -pn $phenoName -cv $covariates -o $outputFolder/chr_X.gz -test
